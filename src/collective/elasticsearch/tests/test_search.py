...
class TestQueries(BaseFunctionalTest):
    ...
    def test_text_index_query(self):
        for idx in range(5):
            createObject(self.portal, "Document", "page%i" % idx, title="Page %i" % idx)
            # should not show up in results
        events = []
        for idx in range(5):
            event = createObject(
                self.portal, "Event", "event%i" % idx, title="Some Event %i" % idx
            )
            events.append(event)
...
if HAS_ATCONTENTTYPES:
    from collective.elasticsearch.testing import (
        ElasticSearch_FUNCTIONAL_TESTING_AT,
    )  # noqa

    EVENT_KLASS_AT = "Products.ATContentTypes.interfaces.event.IATEvent"
    DOCUMENT_KLASS_AT = (
        "Products.ATContentTypes.interfaces.document.IATDocument"  # noqa
    )
